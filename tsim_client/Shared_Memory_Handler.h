...
protected:
	...
	std::string str_notification_lock{ "shm_notification" + str_lock };
...
void set_lock(std::string const & key, int time_out = 1000)
{
	std::string _key = str_lock + key;

	bip::named_mutex shm_mutex{ bip::open_or_create,  _key.c_str() };
	shm_mutex.lock();
...
void release_lock(std::string const & key)
{
	std::string _key = str_lock + key;
	bip::named_mutex shm_mutex{ bip::open_or_create,  _key.c_str() };
	shm_mutex.unlock();
